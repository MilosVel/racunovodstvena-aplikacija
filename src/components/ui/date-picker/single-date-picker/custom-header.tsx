import { format, getMonth, getYear, parse } from 'date-fns';
import { ChevronLeftIcon, ChevronRightIcon } from 'lucide-react';
import { useEffect, useMemo, useState } from 'react';
import type { ReactDatePickerCustomHeaderProps } from 'react-datepicker';

import { Button } from '@/components/ui/button';
import {
  NativeSelect,
  NativeSelectContent,
  NativeSelectItem,
  NativeSelectTrigger,
  NativeSelectValue,
} from '@/components/ui/form/select';
import { createNumberRange } from '@/utils/common';
import { generateMonths, getLastMonthOfYear } from '@/utils/format-date';

export function CustomHeader({
  date,
  changeYear,
  changeMonth,
  decreaseMonth,
  increaseMonth,
  fromYear,
  toYear,
  monthDate,
  changeDisplayedMonth,
  minDate,
  maxDate,
  displayedMonth,
}: ReactDatePickerCustomHeaderProps & {
  fromYear: number;
  toYear: number;
  changeDisplayedMonth: React.Dispatch<React.SetStateAction<Date>>;
  minDate: Date | undefined;
  maxDate: Date | undefined;
  displayedMonth: Date;
}) {
  const months = generateMonths();
  const years = createNumberRange(fromYear, toYear);

  const currentDate = useMemo(() => {
    return date ? new Date(date) : new Date();
  }, [date]);

  const [selectedYear, setSelectedYear] = useState(format(currentDate, 'yyyy'));
  const [selectedMonth, setSelectedMonth] = useState(
    format(currentDate, 'LLLL'),
  );
  const [isPrevDisabled, setIsPrevDisabled] = useState(false);
  const [isNextDisabled, setIsNextDisabled] = useState(false);

  useEffect(() => {
    const displayedYear = getYear(currentDate);
    const displayedMonthIndex = getMonth(currentDate);
    const selectedMonthIndex = months.indexOf(selectedMonth);

    if (+selectedYear < displayedYear) {
      setSelectedYear(String(+selectedYear + 1));
      setSelectedMonth(months[0]);
      changeYear(+selectedYear + 1);
      changeMonth(0);
    } else if (+selectedYear > displayedYear) {
      setSelectedYear(String(+selectedYear - 1));
      setSelectedMonth(months[11]);
      changeYear(+selectedYear - 1);
      changeMonth(11);
    } else if (selectedMonthIndex < displayedMonthIndex) {
      setSelectedMonth(months[selectedMonthIndex + 1]);
      changeMonth(selectedMonthIndex + 1);
    } else if (selectedMonthIndex > displayedMonthIndex) {
      setSelectedMonth(months[selectedMonthIndex - 1]);
      changeMonth(selectedMonthIndex - 1);
    }
  }, [
    currentDate,
    selectedYear,
    changeYear,
    changeMonth,
    selectedMonth,
    months,
  ]);

  const onSelectMonth = (value: string) => {
    changeMonth(months.indexOf(value));
    setSelectedMonth(value);
  };

  const onSelectYear = (value: string) => {
    changeYear(+value);

    const fullDate = parse(
      `${value} ${selectedMonth}`,
      'yyyy MMMM',
      new Date(),
    );

    changeDisplayedMonth(fullDate);
    setSelectedYear(value);
  };

  const decreaseMonthHandler = () => {
    decreaseMonth();
    let month = monthDate && monthDate.getMonth() - 1;
    if (monthDate && monthDate.getMonth() - 1 === -1) {
      month = 11;
      changeYear(currentDate.getFullYear() - 1);
      setSelectedYear(String(currentDate.getFullYear() - 1));
    }

    setSelectedMonth(months[month as number]);
  };

  const increaseMonthHandler = () => {
    increaseMonth();
    let month = monthDate && monthDate.getMonth() + 1;

    if (monthDate.getMonth() === 11) {
      month = 0;
      changeYear(currentDate.getFullYear() + 1);
      setSelectedYear(String(currentDate.getFullYear() + 1));
      setSelectedMonth(months[0]);
      return;
    }

    setSelectedMonth(months[month as number]);
  };

  useEffect(() => {
    const selectedYear = getYear(currentDate);
    const selectedMonth = currentDate.getMonth();

    const maxDateYear = Math.max(...years);
    const minDateYear = Math.min(...years);

    const minDateMonth = minDate
      ? minDate?.getMonth()
      : getLastMonthOfYear(minDateYear);
    const maxDateMonth = maxDate
      ? maxDate?.getMonth()
      : getLastMonthOfYear(maxDateYear);

    const prevDisabled =
      displayedMonth &&
      selectedYear <= minDateYear &&
      selectedMonth <= minDateMonth;

    const nextDisabled =
      displayedMonth &&
      selectedYear >= maxDateYear &&
      selectedMonth === maxDateMonth;

    setIsPrevDisabled(!!prevDisabled);
    setIsNextDisabled(!!nextDisabled);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentDate, displayedMonth, maxDate, minDate, selectedYear]);

  return (
    <div className="mb-2 flex items-center justify-around gap-3 dark:text-white">
      <Button
        variant="ghost"
        className="ml-1 h-6 w-6 border border-input p-0 focus-visible:ring-ring"
        disabled={isPrevDisabled}
        onClick={decreaseMonthHandler}
      >
        <span className="sr-only">Go to previous page</span>
        <ChevronLeftIcon className="h-4 w-4 text-muted-foreground" />
      </Button>

      <div className="flex items-center gap-x-1">
        <NativeSelect value={selectedYear} onValueChange={onSelectYear}>
          <NativeSelectTrigger className="w-[78px]">
            <NativeSelectValue placeholder={selectedYear} />
          </NativeSelectTrigger>
          <NativeSelectContent position="popper">
            <div className="max-h-40 overflow-y-scroll">
              {years.map((option) => (
                <NativeSelectItem key={option} value={option.toString()}>
                  {option}
                </NativeSelectItem>
              ))}
            </div>
          </NativeSelectContent>
        </NativeSelect>

        <NativeSelect value={selectedMonth} onValueChange={onSelectMonth}>
          <NativeSelectTrigger className="w-[115px]">
            <NativeSelectValue placeholder={selectedMonth} />
          </NativeSelectTrigger>
          <NativeSelectContent position="popper">
            <div className="max-h-40 overflow-y-scroll">
              {months.map((option) => (
                <NativeSelectItem key={option} value={option}>
                  {option}
                </NativeSelectItem>
              ))}
            </div>
          </NativeSelectContent>
        </NativeSelect>
      </div>

      <Button
        variant="ghost"
        onClick={increaseMonthHandler}
        className="mr-1 h-6 w-6 border border-input p-0"
        disabled={isNextDisabled}
      >
        <span className="sr-only">Go to next page</span>
        <ChevronRightIcon className="h-4 w-4 text-muted-foreground" />
      </Button>
    </div>
  );
}
